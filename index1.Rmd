...
The usage of bicycle will be affected by weather, temperature and season. The first part we will explore the relationship between season and Citi Bike.
...
for (i in year){
  for(j in quan){
    ...
    data_location <- paste("/Users/Hongbo/Desktop/Citi-Bike-Usage-in-NYC/data/membership/", 
                           dataname, 
                           ".csv", sep = "")
...
Also, the number of membership will also be affected by season(mainly temperature). Since people are not willing to use bikes during winter months, the total number of membership decreases seems reasonable. 
...
Here are several most popular paths for Citi Bike users:
